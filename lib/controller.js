...
Controller.prototype.executeProviderModule = function(commandString) {
	commandString = commandString.substr(1);
	var cmd = commandString.split(" ");
	var cmdFilename = path.join(__dirname, "command." + this.config.type.toLowerCase() + "." + cmd[0] + ".js");
...
Controller.prototype.connected = function(err) {
	...
	process.stdin.on("data", function (chunk) {
		...
		if (chunk.substring(0,1) === ":"){
			self.executeProviderModule(chunk.replace("\n", "").replace("\r", ""));
		} else {
